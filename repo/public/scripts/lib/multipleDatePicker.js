...
(function (angular) {
  ...
  var multipleDatePicker = function () {
        return {
          ...
          '<div  class="text-center picker-navigate picker-navigate-left-arrow monthcontrol" ' +
          'ng-class="{\'disabled\':disableBackButton}" ng-click="previousMonth()">' +
          '&lt;{{(pMonth.format(\'MMM\')) | uppercase}}<br><br>' +
          '</div>' +
          '<div class="text-center picker-month">' +
          '<div class="picker-monthStyle">{{(month.format(\'MMMM\'))| uppercase}}' +
          '</div><span style="font-size: 11pt;">{{month.format(\'YYYY\')}}</span></div>' +

          '<div class="text-center picker-navigate picker-navigate-right-arrow monthcontrol" ' +
          'ng-class="{\'disabled\':disableNextButton}" ng-click="nextMonth()">' +
          '{{(nMonth.format(\'MMM\')) | uppercase}}&gt;<br><br ></div>' +
          '</div>' +
          '<div class="picker-days-week-row" style="font-size: 12pt;">' +
          '<div class="text-center" ng-repeat="day in daysOfWeek">{{day}}</div>' +
          '</div>' +
          '<div class="picker-days-row" style="font-size: 12pt;">' +
          '<div class="text-center picker-day {{getDayClasses(day)}}" ' +
          'title="{{day.title}}" ng-repeat="day in days" ng-click="toggleDay($event, day)" ' +
          'ng-mouseover="hoverDay($event, day)" ng-mouseleave="dayHover($event, day)">' +
          '{{day ? day.mdp.otherMonth && !showDaysOfSurroundingMonths ? \'&nbsp;\' : day.date.format(\'D\') : \'\'}}</div>' +
          '</div>' +
          '</div>',
          link: function (scope) {
            ...
            var checkNavigationButtons = function () {
                  scope.pMonth = moment(scope.month).subtract(1, 'month'),
                      scope.nMonth = moment(scope.month).add(1, 'month');
                  var today = moment(),
              ...
              if (day.selectable && !prevented) {
                ...
                if (day.mdp.selected) {
                  // scope.ngModel.push(day.date);//
                    scope.ngModel.push(moment(day.date).format('YYYY/MM/DD'));// Date format change based on application requirement - Sushil
                } else {
